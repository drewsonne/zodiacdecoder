...
/** These are Daniel's custom functions for our Dictionary purpose ******************************/

int FillDictionary(CHashTable<int>& Dictionary, std::string FileName, int Scores[], unsigned int MaxWordScore) {
	// FILL DICTIONARY
		std::string TempWord;
		std::ifstream InFile(FileName.c_str());									// text file containing dictionary words
		while (InFile >> TempWord) {
			if (TempWord.length() > MaxWordScore) {
				return (false);
			}
...
int SetCustomWordScore(CHashTable<int> &Dictionary, std::string CustomWord, int CustomScore, int Scores[]) {
		if (Dictionary.GetMember(CustomWord))
			Dictionary.RemoveKey(CustomWord, false);
		if (!Dictionary.AddKey(CustomWord, &Scores[CustomScore]))
			return (false);
		return (true);
};	


int GetWordScore(CHashTable<int> &Dictionary, std::string Word) {
	// Returns word score if exists
	// Returns 0 if word does not exist
		int* ScorePointer;
		int Score;

		ScorePointer = Dictionary.GetMember(Word);
